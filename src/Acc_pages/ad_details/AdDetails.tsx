...
function AdDetails() {
  ...
  return (
    <PageContainer title="SDC Advertisement" description="this is advertisement page">
      <Breadcrumb title="Advertisement" items={BCrumb} />
                ...
                <Grid container spacing={3}>
                  ...
                  <Grid item sm={12} md={6} lg={6}>
                    <Stack direction="row" gap={2} alignItems="center" mb={2}>
                      ...
                      <Typography variant="h6"><Translatable>Start Date</Translatable>: <span style={{color: theme.palette.primary.main}}>{singleAd?.start}</span></Typography>
                    </Stack>
                    <Stack direction="row" gap={2} alignItems="center" mb={2}>
                      <IconCalendar size="21" />
                ...
                <Grid item sm={12} lg={6}>
                  <ChildCard>
                    <Box display={'flex'} justifyContent={'space-between'}>
                      ...
                      <Button variant='text' color={'primary'} onClick={() => setOpenAdCoverModal(true)}>
                        <Translatable>
                          Edit
                        </Translatable>
                      </Button>
                ...
                <Grid item xs={12}  lg={6}>
                  ...
                  <Box display={'flex'} justifyContent={'space-between'}>
                    <Typography variant="h4">
                      ...
                      {singleAd?.images?.length > 0 && <Button variant='text' color={'primary'} onClick={() => setOpenAdImagesModal(true)}>
                        <Translatable>
                          Edit
                        </Translatable>
                      </Button>}
                    </Box>
                    { singleAd?.images?.length ? (
                      <ImageList cols={2}>
                        { singleAd?.images.map((photo: any) => (
                          <Box padding={2} key={photo.id}>
                            <DeletableImage key={photo.id} photo={photo}/>
                          </Box>
                        ))}
                        <Box></Box>
                        </ImageList>
                      ...
                      <Box mt={3} sx={{ width: '100%' }}>
                        ...
                        <Stack  my={'28px'} direction="row" justifyContent="flex-end">
                          <Button sx={{width: {md: '150px', xs: '100%'}}} variant="contained" color="primary" onClick={handleSubmitAdImages}>
                            { submitting ? <Translatable>Processing...</Translatable> : <Translatable>Upload</Translatable> }  
                          </Button>
                        </Stack>
                     </Box>
                    )}
