...
import { IconFile, IconImageInPicture, IconVideo } from '@tabler/icons-react';
...
export default function DocumentSingleRow({el, selection}: Props) {

  const theme = useTheme();
  const [err, setErr] = useState<string>('');
  const [isDeleting, setisDeleting] = useState<boolean>(false);
  
  const elCustomized = docCustomized.find((doc: any) => doc.doc_type === el.type )
  ...
  const mutation = useMutation({
    mutationFn: (values: object) => {
      return api().put(`/document-types/update/${el?.id}`, values);
    },
    onSuccess: () => {
              ...
              <Box sx={{ minWidth: { sm: '100%', md: 600, lg: 700 } }} justifyContent={'center'} alignItems={'center'} width={'100%'}>
                <form onSubmit={formik.handleSubmit}>
                  <Stack 
                    ...
                    sx={{ display: 'flex', flexDirection: 'column', gap: { md: 3 } }}
                  >
