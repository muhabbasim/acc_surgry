...
export default function CardLinkSubRow({ subRowEl }: SubRowProps) {
  ...
  const editMutation = useMutation({
    mutationFn: (values: object) => {
      return api().put(`/user-type-cards/update/${subRowEl?.id}`, values);
    }, 
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: () => {
      return api().delete(`/user-type-cards/${subRowEl?.id}`);
    }, 
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['card_links']})
    }
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        ...
        console.log(res)
        toast.success('Card link updated successfully')
        handleClose()
            ...
            >
              <Box sx={{ width: { xs: 250, md: 400} }} justifyContent={'center'} alignItems={'center'} width={'100%'}>
                    <Box width={'100%'} pt={2}>
                      <Typography>
                        <Translatable>
                          Are you sure you want to delete this card link?
                        </Translatable>
                      </Typography>
